		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="project_name" name="project_name" class="form-control col-md-7 col-xs-12">
				@foreach ($projects as $project)
					<option value="{{ $project->project_name }}"{{ $project->project_name === $tax->project_name ? ' selected' : ''}}>{{ $project->project_name }}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="lot_name" name="lot_name" class="form-control col-md-7 col-xs-12">
				@foreach ($projects as $project)
					<option value="{{ $project->lot_name }}"{{ $project->lot_name === $tax->lot_name ? ' selected' : ''}}>{{ $project->lot_name }}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="lot_type" name="lot_type" class="form-control col-md-7 col-xs-12">
				@foreach ($projects as $project)
					<option value="{{ $project->lot_type }}"{{ $project->lot_type === $tax->lot_type ? ' selected' : ''}}>{{ $project->lot_type }}</option>
				@endforeach
			</select>
		</div>
	</div>
