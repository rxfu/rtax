...
@section('title', '新增标段类型')

@section('content')
<form method="post" action="{{ route('type.save') }}" class="form-horizontal form-label-left">
	{{ csrf_field() }}

	<div class="form-group">
		<label for="name" class="control-label col-md-3 col-sm-3 col-xs-12">类型名称 <span class="required">*</span></label>
		<div class="col-md-6 col-sm-6 col-xs-12">
			<input type="text" class="form-control col-md-7 col-xs-12" id="name" name="name" placeholder="类型名称" value="{{ old('name') }}" required>
		</div>
	</div>
	<div class="form-group">
