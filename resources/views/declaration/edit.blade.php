...
@section('title', '编辑自行申报税')

@section('content')
<form method="post" action="{{ route('declaration.update', $declaration->id) }}" class="form-horizontal form-label-left" enctype="multipart/form-data">
	{{ method_field('put') }}
	{{ csrf_field() }}
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="project" name="project" class="form-control col-md-7 col-xs-12">
				@foreach ($projects as $project)
					<option value="project-{{ $project->id }}"{{ $project->id === $declaration->section->project->id ? ' selected' : ''}}>{{ $project->name }}</option>
				@endforeach
			</select>
		</div>
	</div>
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="type" name="type" class="form-control col-md-7 col-xs-12">
				@foreach ($types as $type)
					<option value="type-{{ $type->id }}"{{ $type->id === $declaration->section->type->id ? ' selected' : ''}}>{{ $type->name }}</option>
				@endforeach
			</select>
		</div>
	</div>
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="section_id" name="section_id" class="form-control col-md-7 col-xs-12">
				@foreach ($sections as $section)
					<option value="{{ $section->id }}" data-chained="project-{{ $section->project_id }}+type-{{ $section->type_id }}"{{ $section->id === $declaration->section_id ? ' selected' : ''}}>{{ $section->name }}</option>
				@endforeach
			</select>
		</div>
	</div>
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<select id="tax_name" name="tax_name" class="form-control col-md-7 col-xs-12">
				@foreach ($rates as $rate)
					<option value="{{ $rate->name }}"{{ $rate->name === $declaration->tax_name ? ' selected' : ''}}>{{ $rate->name }}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<input type="text" class="form-control col-md-7 col-xs-12" id="total" name="total" value="{{ $declaration->total }}"  placeholder="税款金额" required>
		</div>
	</div>
	<div class="form-group">
		<label for="issue_time" class="control-label col-md-3 col-sm-3 col-xs-12">开具时间 <span class="required">*</span></label>
		<div class="col-md-6 col-sm-6 col-xs-12">
			<input type="text" class="form-control col-md-7 col-xs-12 has-feedback-left" id="issue_time" name="issue_time" value="{{ $declaration->issue_time }}"  placeholder="开具时间" required>
	...
	<div class="form-group">
		...
		<div class="col-md-6 col-sm-6 col-xs-12">
			<input type="text" class="form-control col-md-7 col-xs-12" id="number" name="number" value="{{ $declaration->number }}"  placeholder="税票号码" required>
		</div>
	</div>
	<div class="form-group">
